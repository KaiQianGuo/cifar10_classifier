...
def train(model, X_train, y_train, X_val, y_val, num_epochs=20, batch_size=128, learning_rate=1e-2, lr_decay=0.95, print_every=1):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0 and epoch != 0:
            plot_loss_acc(train_loss_history, val_loss_history, train_acc_history, val_acc_history,output_path='/home/zhengweiguo/q2q2p/cifar10_classifier/picture/curve_relu_1.png')

    print("Training complete. Best Val Acc:", best_val_acc)
    model.params = best_params
    with open('/home/zhengweiguo/q2q2p/cifar10_classifier/best_model_relu.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    # 激活函数relu、隐藏层神经元数（hidden_dim）：256、学习率（learning_rate）：0.1、L2 正则系数（reg）：0.0
    net = ThreeLayerNet(input_size, hidden_size, output_size, activation='relu', reg_lambda=0.0)
